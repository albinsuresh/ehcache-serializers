...
The Getting Started document gives you an overview of how to use custom serializers with a cache.
This document explains how you can write a custom serilaizer that works with Ehcache.
Here we discuss the significance of serializer transient and persistent serializers in detail and what they actually 
mean.
Let's get started.

== Serializer types

In the Ehcache getting started document you must have already seen the serializers requiring a single argument constructor
or a double-argument constructor or both based on the type of the cache they are used in.
The doc further explains that the serializer with the single-argument constructor is fit to be used with transient 
caches and the ones with the double-argument constructor can be used with persistent caches. An implementation having
both the constructors can be used with both _persistent_ and _transient_ caches.
Hmm... So what does that really mean?
If you look at the custom serializer implementations in the GettingStarted samples they are all have both the costructors
and if you look at the code they don't do anything different.
It's all standard java serialization.
So what difference does the constructors make?
...
----
include::{sourcedir}/test/java/org/ehcache/demo/serializer/DumbTransientStringSerializer.java[tag=transientSerializer]
----
...
Because the serializer that you provided fails in retrieving that data.
...
But using the empty state the  serializer will not able to map that value to a fruit name, and so it throws.
...
and restore it when the cache is re-initialized.
...
`FileBasedPersistenceContext` as an argument, in addition to class loader argument.
The use of the `FileBasedPersistenceContext` argument is optional.
But the presence of this doubl-argument constructor is a strict requirement for persistent caches.
When the cache using this serializer is initialized, this 2-argument constructor is used to instantiate the serializer.
Have a look at this dummy implementation of a persistent serializer.
It is just an extension of the same old transient serializer with the persistent stuff wired in.

[source,java,indent=0]
----
include::{sourcedir}/test/java/org/ehcache/demo/serializer/DumbPersistentStringSerializer.java[tag=persistentSerializer]
----
...
There is no specific reason for me to pick *kryo* in my example.
I don't claim that kryo is the best serialization framework out there and I haven't really done a performance comparison
either.
The sole reason behind the choice was kryo's popularity and nothing else.
