...
popularity and nothing else. So here is the kryo based custom serializer:

[source,java,indent=0]
----
include::{sourcedir}/test/java/org/ehcache/demo/serializer/KryoStringSerializer.java[tag=thirdPartySerializer]
----

And here is the usage of the same:
...
<1> The only change from the previous sample is the usage of `KryoStringSerializer` here.
