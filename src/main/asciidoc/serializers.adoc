...
This document explains how you can write a transient/persistent custom serializer that works with Ehcache.
Here we discuss the significance of transient serializers and persistent serializers in detail through a practical example

== Serializer types

As indicated in the Ehcache documentation, serializers require a single argument constructor or a double-argument 
constructor or both based on the type of the cache they are used in.
